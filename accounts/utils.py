...
def get_all_post_photos(request, user_pk):
	user = User.objects.get(pk=user_pk)
	posts = get_user_post_feeds(request, user_pk)
	photo_list = [post_img for post in posts for post_img in post.postimage_set.all()]
	return sorted(photo_list, key=lambda x:x.created, reverse=True)
